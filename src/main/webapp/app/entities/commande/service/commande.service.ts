...
export class CommandeService {
  ...
  protected convertDateFromClient(commande: ICommande): ICommande {
    return Object.assign({}, commande, {
      dateCommande: commande.dateCommande?.isValid() ? commande.dateCommande.toJSON() : undefined,
      dateSortie: commande.dateSortie?.isValid() ? commande.dateSortie.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commande: ICommande) => {
        commande.dateCommande = commande.dateCommande ? dayjs(commande.dateCommande) : undefined;
        commande.dateSortie = commande.dateSortie ? dayjs(commande.dateSortie) : undefined;
      });
    }
    return res;
  }
